...
void FlutterWebRTC::HandleMethodCall(
  ...
  if (method_call.method_name().compare("initialize") == 0) {
    ...
    audio_device_->SpeakerVolume(currentVol);
    // Set speaker volume to maximum (65535 is typical WASAPI max)
    audio_device_->SetSpeakerVolume(65535);
    uint32_t afterVol = 0;
    audio_device_->SpeakerVolume(afterVol);
    ...
    info[EncodableValue("playoutDevices")] = EncodableValue((int64_t)audio_device_->PlayoutDevices());
