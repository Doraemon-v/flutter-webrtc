...
- (instancetype)initWithChannel:(FlutterMethodChannel*)channel
  ...
  self.virtualBackgroundProcessors = [NSMutableDictionary new];
...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
  ...
  } else if ([@"getVirtualBackgroundCapabilities" isEqualToString:call.method]) {
    result([MLVirtualBackgroundProcessor capabilities]);
  } else if ([@"setVirtualBackground" isEqualToString:call.method]) {
    NSDictionary* argsMap = call.arguments;
    NSString* trackId = argsMap[@"trackId"];
    id<LocalTrack> localTrack = self.localTracks[trackId];
    if (trackId.length == 0 || localTrack == nil || ![localTrack isKindOfClass:[LocalVideoTrack class]]) {
      result(@{
        @"applied" : @NO,
        @"reason" : @"未找到可处理的本地视频轨道"
      });
      return;
    }

    LocalVideoTrack* videoTrack = (LocalVideoTrack*)localTrack;
    MLVirtualBackgroundProcessor* processor = self.virtualBackgroundProcessors[trackId];
    if (processor == nil) {
      processor = [[MLVirtualBackgroundProcessor alloc] init];
      [videoTrack addProcessing:processor];
      self.virtualBackgroundProcessors[trackId] = processor;
    }

    NSError* error = nil;
    BOOL applied = [processor updateWithOptions:argsMap error:&error];
    result(@{
      @"applied" : @(applied),
      @"reason" : error.localizedDescription ?: [NSNull null]
    });
...
- (void)dealloc {
  [_virtualBackgroundProcessors removeAllObjects];
  _virtualBackgroundProcessors = nil;
...
- (void)detachVirtualBackgroundProcessorForTrackId:(NSString*)trackId {
  if (trackId.length == 0) return;

  MLVirtualBackgroundProcessor* processor = self.virtualBackgroundProcessors[trackId];
  id<LocalTrack> localTrack = self.localTracks[trackId];
  if (processor != nil && [localTrack isKindOfClass:[LocalVideoTrack class]]) {
    [(LocalVideoTrack*)localTrack removeProcessing:processor];
  }
  [self.virtualBackgroundProcessors removeObjectForKey:trackId];
}
